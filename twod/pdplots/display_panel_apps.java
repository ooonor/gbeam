...
public class display_panel_apps
{
    ...
    /**
     * Constructor for objects of class display_panel_apps
     * which is a lot like basicframe but doesn't have a specific jpanel extension
     * (that is handled in the main program)
     */
    public display_panel_apps(double degradation_in, int n, double[] x, double[] y, int n_title_lines, String[] title, 
    ...
    /**
     * this is now involke explicitly from the main program
    */
    public void paint_panel(Graphics g)
    {
        int bot1;
        int bot2;
        int miny = 100;
    ...
    private void paint_legend(Graphics2D g, int maxx, int miny, int chgt) {
        String txt1 = "Ideal";
        String txt2 = "Actual";
        String txt3 = "(Degradation=" + this.degradation + " dB)";
        ...
        g.drawString(txt3, maxx - metrics.stringWidth(txt3), miny + 2*hgt);
    }
    private void paint_titles(Graphics2D g, int minx, int miny, int chgt) {
        ...
        for (int i = 0; i < this.n_title; i++){
           ...
           this.tbot = this.tbot + hgt;
        }
        // if there are fewer than 3 title lines, increase tbot to accomodate legend
        if(this.n_title < 3) this.tbot = miny + 3*hgt;
    }
    ...
    public void paint_cursor(Graphics g) {
        Graphics2D g2 = (Graphics2D)g;
        if (this.cur == 2) {
            paint_pdpd_cursor(g2, this.g_pdpd);
        }
        else if(this.cur ==1) {
            paint_pdse_cursor(g2, this.g_pdse);
        }
    }
    private void paint_pdse_cursor(Graphics2D g, grph gg) {
            g.setColor(this.customRed);
    ...
    private void paint_pdpd_cursor(Graphics2D g, grph gg) {
